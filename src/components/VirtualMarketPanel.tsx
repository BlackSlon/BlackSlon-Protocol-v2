  ...
  return (
    <div className="flex flex-col h-full bg-black font-mono text-white p-0">
      ...
      <div className="flex-grow px-6 pb-6 flex flex-col min-h-0">
        {/* 2. TYTUŁ INSTRUMENTU (Brązowy) */}
        <div className="p-4 bg-gradient-to-b from-black to-gray-950">
          <div className="text-[12px] tracking-widest text-amber-700 mb-1">
          ...
          <div className="flex-grow overflow-hidden">
            {buyOrders.map((o, i) => (
              <div key={i} className="grid grid-cols-3 py-0.5 px-4 border-b border-gray-900/50 hover:bg-green-500/10 transition-all group">
                ...
                <div className={i === 0 ? "text-xl text-green-500 tracking-tighter leading-none text-right self-center animate-pulse" : "text-right text-[11px] text-green-500 self-center"}>{o.price.toFixed(2)}</div>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="flex-grow overflow-hidden">
            {sellOrders.map((o, i) => (
              <div key={i} className="grid grid-cols-3 py-0.5 px-4 border-b border-gray-900/50 hover:bg-red-500/10 transition-all group">
                <div className={i === 0 ? "text-xl text-red-500 tracking-tighter leading-none self-center animate-pulse" : "text-left text-[11px] text-red-500 self-center"}>{o.price.toFixed(2)}</div>
