  ...
  return (
    <div className="min-h-screen bg-black text-white p-8">
      <nav className="max-w-7xl mx-auto mb-12 flex justify-between items-center border-b border-gray-900 pb-6">
        <Link href="/" className="text-gray-600 hover:text-white transition-all text-[10px] font-normal uppercase tracking-widest">← Return to Dashboard</Link>
      ...
      <main className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-12">
        <div className="lg:col-span-2 space-y-8">
          <div className="flex justify-between items-end">
            <div>
              <h1 className="text-5xl font-normal italic tracking-tighter leading-none uppercase">{market.name}</h1>
              <div className="text-yellow-500 font-mono text-xs mt-4 uppercase tracking-[0.3em]">{market.type} IPT Participant</div>
            </div>
            <div className="text-right">
              <div className="text-[10px] text-gray-600 uppercase mb-1 font-normal tracking-widest text-blue-400">Current BSEI Value</div>
              <div className="text-6xl font-mono font-normal leading-none">{currentPrice.toFixed(4)}</div>
            </div>
          </div>
        ...
        <div className="bg-[#080808] border border-gray-900 p-8 h-fit shadow-xl">
          <h2 className="text-[10px] font-normal text-gray-600 uppercase mb-10 tracking-[0.4em] border-b border-gray-900 pb-4 text-center">Execution Panel</h2>
          <div className="space-y-6">
            ...
            />
            <button onClick={() => setS(s + orderAmount)} className="w-full py-6 bg-yellow-500 text-black font-normal uppercase text-xs hover:bg-yellow-400 transition-all tracking-widest">Buy IPT (Long)</button>
            <button onClick={() => setS(s - orderAmount)} className="w-full py-6 border border-gray-800 text-gray-600 font-normal uppercase text-xs hover:border-red-600 hover:text-red-600 transition-all tracking-widest">Sell IPT (Short)</button>
          </div>
        </div>
      </main>
    </div>
