...
const MARKET_INFO: Record<string, MarketInfo> = {
  'BS-P-PL': { marketName: 'Polish Power Wholesale Market', exchangeName: 'Polish Energy Exchange (TGE)', commodity: 'power', token: 'BS-P-PL' },
  'BS-P-DE': { marketName: 'German Power Wholesale Market', exchangeName: 'EEX',                         commodity: 'power', token: 'BS-P-DE' },
  'BS-P-NO': { marketName: 'Nordic Power Wholesale Market', exchangeName: 'Nord Pool NO',                commodity: 'power', token: 'BS-P-NO' },
  'BS-P-UK': { marketName: 'UK Power Wholesale Market',     exchangeName: 'N2EX UK',                     commodity: 'power', token: 'BS-P-UK' },
  'BS-G-NL': { marketName: 'Dutch TTF Gas Wholesale Market', exchangeName: 'ICE Endex TTF',              commodity: 'gas',   token: 'BS-G-NL' },
  'BS-G-DE': { marketName: 'German THE Gas Wholesale Market', exchangeName: 'EEX',                       commodity: 'gas',   token: 'BS-G-DE' },
  'BS-G-PL': { marketName: 'Polish Gas Wholesale Market',   exchangeName: 'Polish Energy Exchange (TGE)', commodity: 'gas',   token: 'BS-G-PL' },
  'BS-G-BG': { marketName: 'Bulgarian Gas Wholesale Market', exchangeName: 'Balkan Gas Hub',             commodity: 'gas',   token: 'BS-G-BG' },
}
...
export function getMarketTooltips(marketId: string) {
  ...
  return {
    ...
    anchor:
      `The Anchor for the ${m.marketName} is calculated as a weighted average of physical spot and forward prices — 10% Day-Ahead spot, 40% Front Month, 25% Front Quarter, and 25% Calendar Year — smoothed over the last three trading days using a recursive 50/25/25 formula. This three-day smoothing prevents any single-day spike or manipulation from instantly shifting the corridor. The Anchor moves with the ${m.marketName} trend — if the price of ${m.commodity} rises for several consecutive days, the Anchor rises with them and the entire corridor shifts upward. It is the protocol's mathematical connection to ${m.marketName} reality.`,

    floor:
      `The Floor is the lower boundary of the BSSZ corridor, set at Anchor minus 10%. It is deliberately tight by design. Because the Anchor continuously tracks the physical market trend, the Floor does not cap the upside — it moves up with the ${m.marketName}. Its purpose is to protect long-term BlackSlon token holders from sudden, irrational downward spikes that bear no relation to the underlying physical trend. Positions cannot be opened or settled below this level. The Floor is a shield against manipulation and panic-driven sell-offs, not a constraint on legitimate price discovery.`,

    cap:
      `The Cap is the upper boundary of the BSSZ corridor, set at Anchor plus 20%. It is wider than the Floor by design, reflecting the asymmetric nature of ${m.commodity} price shocks — prices spike upward far more violently than they collapse. A participant who wants to buy at elevated prices may do so within the corridor. If the ${m.marketName} sustains that move, the Anchor will follow over subsequent days and the Cap will shift upward accordingly. The asymmetry is not arbitrary — it mirrors the structural behaviour of European ${m.commodity} markets during geopolitical and supply-side shocks.`,

    fm:
      `Front Month is the current settlement price of the front-month ${m.commodity} contract on the ${m.exchangeName}, verified by oracles. It is the most liquid and widely traded segment of the forward curve, carrying 40% weight in the Anchor for the ${m.marketName}. This is the product professional market participants trade every day.`,

    bsszVsFwd:
      `This section compares the Anchor for the ${m.marketName} against the current Front Month price on the ${m.exchangeName}. The difference reveals whether the physical ${m.marketName} is trading at a premium or discount relative to the Anchor. A positive spread indicates the physical market has moved ahead of the Anchor, suggesting upward momentum not yet fully absorbed into the corridor. A negative spread indicates the physical market has pulled back. This comparison is updated daily and gives participants a real-time view of how the BSSZ corridor relates to live physical market conditions.`,

    spread:
      `The Spread shows the difference between the current Front Month price on the ${m.exchangeName} and the Anchor for the ${m.marketName}, expressed both in percentage terms and in absolute EUR per 100 kWh. A positive spread means the ${m.marketName} is trading above the Anchor — the forward curve has moved faster than the corridor. A negative spread means the physical ${m.marketName} has retreated below the Anchor. The spread is a leading indicator: sustained positive spreads signal that the Anchor will follow upward in subsequent days as the recursive formula absorbs the new price levels. It is the gap between where the market is and where the corridor is anchored.`,

    orderBook:
      `The Order Book is the live limit order book where participants trade ${m.token} tokens directly with each other at supply/demand-driven prices. All bids and asks are visible to all participants in real time — there is no preferential order routing or hidden liquidity. Orders are constrained by the BSSZ corridor: no order outside the range of Anchor minus 10% to Anchor plus 20% can be placed or executed. The matching engine uses price-time priority — the best-priced order executes first, and among equal prices, the earliest order has priority.`,

    liquidity:
      `The Liquidity indicator shows the total volume of trades executed on the BlackSlon ${m.marketName} over the last 24 hours — a direct measure of market activity and depth. Higher volume signals an active, liquid market where positions can be entered and exited at fair prices with minimal slippage. Lower volume may indicate wider spreads and reduced price discovery efficiency. Unlike traditional energy exchanges that operate within fixed daily windows, BlackSlon Protocol runs 24/7/365 — liquidity is continuously generated across all time zones, including weekends and holidays when physical risk events most commonly occur.`,

    bsei:
      `The BlackSlon Energy Index (BSEI) is the transaction-derived settlement benchmark for the BlackSlon ${m.marketName}, calculated exclusively from executed ${m.token} trades. It uses a three-tier Segmented Rolling VWAP over a 72-hour window: the most recent 24 hours carry 50% weight, the preceding 24–48 hours carry 25%, and the oldest 48–72 hours carry 25%. This structure makes single-session manipulation economically unviable — to move the BSEI, an actor would need to dominate trading volume across three consecutive days. The BSEI is the single source of data for Mark-to-Market valuation and margin requirements across BlackSlon Markets.`,

    collateral:
      `The Collateral Configuration defines the ratio of €BSR to eEURO used as margin to open a new position (BUY or SELL ${m.token}). Every position in the BlackSlon Protocol is backed by a dual-asset collateral portfolio. The higher the proportion of €BSR in your collateral, the lower your required % margin and trading fees — and the higher your available leverage. At 100% €BSR, the margin requirement drops to 25% for long positions and fees fall to 0.20%. At 10% €BSR, the margin requirement is 50% and fees are 1.00%. This tiered system aligns protocol sustainability with user capital efficiency — staking more €BSR signals long-term commitment and is rewarded with better trading conditions. When closing a position, the closing fee is a flat 0.20% of the position's notional value, regardless of your €BSR ratio.`,

    requiredMargin:
      `The Required Margin is the minimum collateral that must be deposited and locked to open and maintain your position. It is calculated as a percentage of the position's notional value, determined by your Collateral Configuration tier and the direction of your trade. Short (SELL) positions require twice the margin of long (BUY) positions — by design, not by symmetry. BlackSlon is built primarily for long-term token holders and participants who want structural exposure to European energy markets. The higher short margin is a deliberate architectural choice to protect long holders from coordinated short squeezes and to preserve the protocol's role as an energy-linked store of value. The margin is locked in your BlackSlon Vault for the duration of the position. If your portfolio equity falls below 50% of aggregate required margin, the Smart Incremental Liquidation mechanism activates — reducing positions in 10% steps rather than forcing a full close.`,

    tradingTerminal:
      `The BlackSlon Trading Terminal is the order entry interface for buying and selling ${m.token}. Start by selecting BUY or SELL — this determines the required margin and fee structure for your order. Then set your order price within the BSSZ corridor — any price outside the BSSZ will be rejected before reaching the matching engine. Set your quantity in tokens, where 1 token represents 100 kWh of energy. In the Collateral Configuration section, choose your €BSR to eEURO ratio — the higher the €BSR proportion, the lower your required margin and fees. The terminal will calculate your exact €BSR deposit, required margin, and eEURO deposit in real time. Once you are satisfied with the parameters, CONFIRM your ORDER. Your order is then placed directly in the BlackSlon Order Book and matched against available counterparties using price-time priority. Active orders are visible below and can be cancelled at any time before execution.`,
