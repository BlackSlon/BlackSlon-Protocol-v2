<div align="center">
  ...
  <div style="font-family: Arial, Helvetica, sans-serif; font-weight: normal; font-size: 1.5em; margin-top: -10px;">White Paper v2.0</div>
</div>
...
**Version:** 2.0 | **Status:** APPROVED | **Date:** March 2026  
...
The conservative valuation of €BSR held within the protocol's own reserve positions (not user collateral — that is already counted in Micro Health Factor). A haircut $h_{BSR}$ is applied to account for €BSR price volatility, scaled to the current solvency tier:

$$h_{BSR} = \begin{cases} 10\% & \text{if } H_{solv} > 1.15 \text{ (Tier I)} \\ 15\% & \text{if } 1.05 \leq H_{solv} \leq 1.15 \text{ (Tier II)} \\ 25\% & \text{if } 1.00 \leq H_{solv} < 1.05 \text{ (Tier III)} \\ 35\% & \text{if } H_{solv} < 1.00 \text{ (Tier IV)} \end{cases}$$

As solvency deteriorates, the protocol applies progressively higher haircuts to €BSR reserves — reducing their recognised value in the asset base and creating an automatic deleveraging effect that strengthens the denominator coverage ratio.
